  ...
  def message(assigns) do
    ...
    <div id={@dom_id} class="group relative flex px-4 py-3">
      <div 
        ...
        class="absolute top-4 right-4 group-hover:block bg-white shadow-sm px-2 pb-1 rounded border border-px border-slate-300 gap-1"
      >
        <button
          :if={!@in_thread?}
        ...
        <div class="-mt-1">
          ...
          >
            <%= for {emoji, count, me?} <- enumerate_reactions(@message.reactions, @current_user) do %>
              <button
                class={[
                  "flex items-center pl-2 pt-2 pr-2 h-6 rounded-full text-xs",
